...
class LoginHandler(BaseHandler):
    ...
    def post(self):
        if self.application.db.is_closed():
...
class SocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, raw_data):
        ...
        elif message["type"] == "delete_friend":
            ret["type"] = message['type']
            me=User.select().where(User.id==self.user_id).get()
            friend=User.select().where(User.id==message["friend_id"]).get()
            mygroup = ReferenceGroup.select().where(
                ReferenceGroup.id == message["group_id"]).get()
            myrf=Reference.select().where(Reference.group==mygroup,Reference.friend_user==friend).get()
            friend_groups=ReferenceGroup.select().where(
                ReferenceGroup.user==friend
            )
            for group in friend_groups:
                friend_rfs=Reference.select().where(Reference.group==group,Reference.friend_user==me)
                if len(friend_rfs):
                    friend_rf=friend_rfs.get()
                    friend_group=group
                    break
            myrf.delete_instance()
            if mygroup.name!="默认分组" and (not len(Reference.select().where(Reference.group==mygroup))):
                mygroup.delete_instance()
            friend_rf.delete_instance()
            if friend_group.name!="默认分组" and (not len(Reference.select().where(Reference.group==friend_group))):
                friend_group.delete_instance()
            other_socket = self.application.socketList.getOneUserById(message[
                                                                          "friend_id"])
            if other_socket is not None:
                other_socket.write_message(json.dumps(ret))
